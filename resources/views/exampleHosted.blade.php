...
$name = $userC->name;
$studentID = $stuC->sid;
$studentPhone = $userC->mobile;
//$userStu = \App\Models\Teacher::find(session()->get('loginUser'))->first();
//dd($userStu);
$degree = $stuC->degree;
//dd($degree);
$level = $stuC->level;
$semester = $stuC->semester;
$semesterFees = DB::select('select * from enrollments where enrollments.degree=? AND enrollments.level=? AND enrollments.semester=?',[$degree,$level,$semester]);
//$semesterFees = DB::select('select * from semester_fees where department=?',[$student->department]);
//dd($semesterFees);
$facultyFee = 100;
if($semester == "I") $hallFee = 1000;
else $hallFee = 0;
...
<div class="container">
    ...
    <div class="row">
        ...
        <div class="col-md-8 order-md-1">
            ...
            <form action="{{ url('/pay', ['uid' => $userC->id]) }}" method="POST" class="needs-validation">
